...
namespace moodycamel {
	...
	// Default traits for the AtomicMpMcQueue. To change some of the
	// traits without re-implementing all of them, inherit from this
	// struct and shadow the declarations you wish to be different;
	// since the traits are used as a template type parameter, the
	// shadowed declarations will be used where defined, and the defaults
	// otherwise.
	struct AtomicMpMcQueueDefaultTraits
	{
		...
		// Only affects instances of the BlockingAtomicMpMcQueue.
	...
	template<typename T, typename Traits> class AtomicMpMcQueue;
	template<typename T, typename Traits> class BlockingAtomicMpMcQueue;
	class AtomicMpMcQueueTests;


	namespace details
	{
		struct AtomicMpMcQueueProducerTypelessBase
		{
			AtomicMpMcQueueProducerTypelessBase* next;
			std::atomic<bool> inactive;
			ProducerToken* token;

			AtomicMpMcQueueProducerTypelessBase()
	...
	struct ProducerToken
	{
		...
		explicit ProducerToken(AtomicMpMcQueue<T, Traits>& queue);

		template<typename T, typename Traits>
		explicit ProducerToken(BlockingAtomicMpMcQueue<T, Traits>& queue);
	...
	private:
		template<typename T, typename Traits> friend class AtomicMpMcQueue;
		friend class AtomicMpMcQueueTests;

	protected:
		details::AtomicMpMcQueueProducerTypelessBase* producer;
	};
	...
	{
		template<typename T, typename Traits>
		explicit ConsumerToken(AtomicMpMcQueue<T, Traits>& q);

		template<typename T, typename Traits>
		explicit ConsumerToken(BlockingAtomicMpMcQueue<T, Traits>& q);
	...
	private: // but shared with AtomicMpMcQueue
		std::uint32_t initialOffset;
		std::uint32_t lastKnownGlobalOffset;
		std::uint32_t itemsConsumedFromCurrent;
		details::AtomicMpMcQueueProducerTypelessBase* currentProducer;
		details::AtomicMpMcQueueProducerTypelessBase* desiredProducer;
	};

	// Need to forward-declare this swap because it's in a namespace.
	// See http://stackoverflow.com/questions/4492062/why-does-a-c-friend-class-need-a-forward-declaration-only-in-other-namespaces
	template<typename T, typename Traits>
	inline void swap(typename AtomicMpMcQueue<T, Traits>::ImplicitProducerKVP& a, typename AtomicMpMcQueue<T, Traits>::ImplicitProducerKVP& b) MOODYCAMEL_NOEXCEPT;


	template<typename T, typename Traits = AtomicMpMcQueueDefaultTraits>
...
#pragma warning(pop)
	...
	public:
		...
		// memory barrier).
		explicit AtomicMpMcQueue(size_t capacity = 32 * BLOCK_SIZE)
		...
		AtomicMpMcQueue(size_t minCapacity, size_t maxExplicitProducers, size_t maxImplicitProducers)
			: producerListTail(nullptr),
			producerCount(0),
			initialBlockPoolIndex(0),
			nextExplicitConsumerId(0),
		...
		AtomicMpMcQueue(AtomicMpMcQueue const&) MOODYCAMEL_DELETE_FUNCTION;
		AtomicMpMcQueue& operator=(AtomicMpMcQueue const&) MOODYCAMEL_DELETE_FUNCTION;
		...
		AtomicMpMcQueue(AtomicMpMcQueue&& other) MOODYCAMEL_NOEXCEPT
		...
		inline AtomicMpMcQueue& operator=(AtomicMpMcQueue&& other) MOODYCAMEL_NOEXCEPT
		{
		...
		inline void swap(AtomicMpMcQueue& other) MOODYCAMEL_NOEXCEPT
		{
			swap_internal(other);
		}

	private:
		AtomicMpMcQueue& swap_internal(AtomicMpMcQueue& other)
		{
			if (this == &other) {
				return *this;
			}
	...
	private:
		...
		{
			return static_cast<ExplicitProducer*>(token.producer)->AtomicMpMcQueue::ExplicitProducer::template enqueue<canAlloc>(std::forward<U>(element));
		}
		...
		inline bool inner_enqueue(U&& element)
		{
			...
			return producer == nullptr ? false : producer->AtomicMpMcQueue::ImplicitProducer::template enqueue<canAlloc>(std::forward<U>(element));
		}

		template<AllocationMode canAlloc, typename It>
		inline bool inner_enqueue_bulk(producer_token_t const& token, It itemFirst, size_t count)
		{
			return static_cast<ExplicitProducer*>(token.producer)->AtomicMpMcQueue::ExplicitProducer::template enqueue_bulk<canAlloc>(itemFirst, count);
		}
		...
		{
			auto producer = get_or_add_implicit_producer();
			return producer == nullptr ? false : producer->AtomicMpMcQueue::ImplicitProducer::template enqueue_bulk<canAlloc>(itemFirst, count);
		}
	...
	private:
		...
		struct ProducerBase : public details::AtomicMpMcQueueProducerTypelessBase
		{
			ProducerBase(AtomicMpMcQueue* parent_, bool isExplicit_) :
				tailIndex(0),
				headIndex(0),
		...
		{
			explicit ExplicitProducer(AtomicMpMcQueue* parent_) :
			...
			~ExplicitProducer()
			{
				...
				if (this->tailBlock != nullptr) {		// Note this means there must be a block index too
					...
					do {
						block = block->next;
						if (block->AtomicMpMcQueue::Block::template is_empty<explicit_context>()) {
							continue;
						}
			...
			inline bool enqueue(U&& element)
			{
				...
				if ((currentTailIndex & static_cast<index_t>(BLOCK_SIZE - 1)) == 0) {
					...
					if (this->tailBlock != nullptr && this->tailBlock->next->AtomicMpMcQueue::Block::template is_empty<explicit_context>()) {
						// We can re-use the block ahead of us, it's empty!					
						this->tailBlock = this->tailBlock->next;
						this->tailBlock->AtomicMpMcQueue::Block::template reset_empty<explicit_context>();
						...
						auto newBlock = this->parent->AtomicMpMcQueue::template requisition_block<allocMode>();
						if (newBlock == nullptr) {
							return false;
						}
#ifdef MCDBGQ_TRACKMEM
						newBlock->owner = this;
#endif
						newBlock->AtomicMpMcQueue::Block::template reset_empty<explicit_context>();
						if (this->tailBlock == nullptr) {
					...
					if ((details::likely)(details::circular_less_than<index_t>(myDequeueCount - overcommit, tail))) {
						...
						if (!MOODYCAMEL_NOEXCEPT_ASSIGN(T, T&&, element = std::move(el))) {
							...
							struct Guard {
								...
								{
									(*block)[index]->~T();
									block->AtomicMpMcQueue::Block::template set_empty<explicit_context>(index);
								}
							} guard = { block, index };
			...
			bool MOODYCAMEL_NO_TSAN enqueue_bulk(It itemFirst, size_t count)
			{
				...
				if (blockBaseDiff > 0) {
					...
					while (blockBaseDiff > 0 && this->tailBlock != nullptr && this->tailBlock->next != firstAllocatedBlock && this->tailBlock->next->AtomicMpMcQueue::Block::template is_empty<explicit_context>()) {
					...
					while (blockBaseDiff > 0) {
						...
						newBlock->owner = this;
#endif
						newBlock->AtomicMpMcQueue::Block::template set_all_empty<explicit_context>();
						if (this->tailBlock == nullptr) {
							newBlock->next = newBlock;
						}
						else {
					...
					while (true) {
						block->AtomicMpMcQueue::Block::template reset_empty<explicit_context>();
							...
							if (MOODYCAMEL_NOEXCEPT_ASSIGN(T, T&&, details::deref_noexcept(itemFirst) = std::move((*(*block)[index])))) {
								...
								MOODYCAMEL_TRY{
									...
									do {
										...
										block->AtomicMpMcQueue::Block::template set_many_empty<explicit_context>(firstIndexInBlock, static_cast<size_t>(endIndex - firstIndexInBlock));
		...
		{
			ImplicitProducer(AtomicMpMcQueue* parent_) :
			...
			inline bool enqueue(U&& element)
			{
				...
				if ((currentTailIndex & static_cast<index_t>(BLOCK_SIZE - 1)) == 0) {
					...
					newBlock->owner = this;
#endif
					newBlock->AtomicMpMcQueue::Block::template reset_empty<implicit_context>();
					...
					if ((details::likely)(details::circular_less_than<index_t>(myDequeueCount - overcommit, tail))) {
						...
						if (!MOODYCAMEL_NOEXCEPT_ASSIGN(T, T&&, element = std::move(el))) {
							...
							struct Guard {
								...
								{
									(*block)[index]->~T();
									if (block->AtomicMpMcQueue::Block::template set_empty<implicit_context>(index)) {
			...
			bool enqueue_bulk(It itemFirst, size_t count)
			{
				...
				if (blockBaseDiff > 0) {
					...
					do {
						...
						if (full || !(indexInserted = insert_block_index_entry<allocMode>(idxEntry, currentTailIndex)) || (newBlock = this->parent->AtomicMpMcQueue::template requisition_block<allocMode>()) == nullptr) {
...
#pragma warning(pop)
							...
							if (MOODYCAMEL_NOEXCEPT_ASSIGN(T, T&&, details::deref_noexcept(itemFirst) = std::move((*(*block)[index])))) {
								...
								MOODYCAMEL_TRY{
									...
									do {
										...
										if (block->AtomicMpMcQueue::Block::template set_many_empty<implicit_context>(blockStartIndex, static_cast<size_t>(endIndex - blockStartIndex))) {
#ifdef MCDBGQ_NOLOCKFREE_IMPLICITPRODBLOCKINDEX
											debug::DebugLock lock(mutex);
#endif
											entry->value.store(nullptr, std::memory_order_relaxed);
	...
	public:
		struct MemStats {
			...
			friend class AtomicMpMcQueue;

		private:
			static MemStats getFor(AtomicMpMcQueue* q)
			{
				MemStats stats = { 0 };
				...
				for (auto ptr = q->producerListTail.load(std::memory_order_acquire); ptr != nullptr; ptr = ptr->next_prod()) {
					...
					if (implicit) {
						...
						if (tailBlock != nullptr) {
							...
							do {
								++stats.allocatedBlocks;
								if (!block->AtomicMpMcQueue::Block::template is_empty<explicit_context>() || wasNonEmpty) {
									++stats.usedBlocks;
				...
				stats.queueClassBytes += sizeof(AtomicMpMcQueue);
	...
	private:
		...
		friend void moodycamel::swap(typename AtomicMpMcQueue<XT, XTraits>::ImplicitProducerKVP&, typename AtomicMpMcQueue<XT, XTraits>::ImplicitProducerKVP&) MOODYCAMEL_NOEXCEPT;
		...
		void swap_implicit_producer_hashes(AtomicMpMcQueue& other)
		{
			MOODYCAMEL_CONSTEXPR_IF(INITIAL_IMPLICIT_PRODUCER_HASH_SIZE == 0) {
				return;
			}
...
#ifdef MOODYCAMEL_CPP11_THREAD_LOCAL_SUPPORTED
					producer->threadExitListener.callback = &AtomicMpMcQueue::implicit_producer_thread_exited_callback;
	...
	template<typename T, typename Traits>
	ProducerToken::ProducerToken(AtomicMpMcQueue<T, Traits>& queue)
	...
	template<typename T, typename Traits>
	ProducerToken::ProducerToken(BlockingAtomicMpMcQueue<T, Traits>& queue)
		: producer(reinterpret_cast<AtomicMpMcQueue<T, Traits>*>(&queue)->recycle_or_create_producer(true))
	{
		if (producer != nullptr) {
			producer->token = this;
		}
	...
	template<typename T, typename Traits>
	ConsumerToken::ConsumerToken(AtomicMpMcQueue<T, Traits>& queue)
	...
	template<typename T, typename Traits>
	ConsumerToken::ConsumerToken(BlockingAtomicMpMcQueue<T, Traits>& queue)
		: itemsConsumedFromCurrent(0), currentProducer(nullptr), desiredProducer(nullptr)
	{
		initialOffset = reinterpret_cast<AtomicMpMcQueue<T, Traits>*>(&queue)->nextExplicitConsumerId.fetch_add(1, std::memory_order_release);
		lastKnownGlobalOffset = static_cast<std::uint32_t>(-1);
	}

	template<typename T, typename Traits>
	inline void swap(AtomicMpMcQueue<T, Traits>& a, AtomicMpMcQueue<T, Traits>& b) MOODYCAMEL_NOEXCEPT
	{
		a.swap(b);
	}
